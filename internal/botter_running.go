...
func (pInst *cTBotApi) processMessageUser_running(tMsg *tgbotapi.Message) {
	if tMsg.Photo != nil {
		pInst.spi.MessageUserPhoto(tMsg, tMsg.Chat.ID, tMsg.Photo)
	} else if tMsg.Audio != nil {
		pInst.spi.MessageUserAudio(tMsg, tMsg.Chat.ID, tMsg.Audio)
	} else if tMsg.Video != nil {
		pInst.spi.MessageUserVideo(tMsg, tMsg.Chat.ID, tMsg.Video)
	}
	if tMsg.Text != "" {
		pInst.spi.MessageUserText(tMsg, tMsg.Chat.ID, tMsg.Text)
	}
